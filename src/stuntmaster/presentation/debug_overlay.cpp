...
constexpr Glyph glyph(char character) noexcept {
    ...
    // High-value punctuation for the host license overlay. Unknown characters
    // still fall through to a blank glyph, so an omission degrades legibility
    // rather than breaking layout.
    case '.': return {0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x0CU, 0x0CU};
    case ',': return {0x00U, 0x00U, 0x00U, 0x00U, 0x0CU, 0x04U, 0x08U};
    case ':': return {0x00U, 0x0CU, 0x0CU, 0x00U, 0x0CU, 0x0CU, 0x00U};
    case ';': return {0x00U, 0x0CU, 0x0CU, 0x00U, 0x0CU, 0x04U, 0x08U};
    case '\'': return {0x04U, 0x04U, 0x08U, 0x00U, 0x00U, 0x00U, 0x00U};
    case '"': return {0x0AU, 0x0AU, 0x0AU, 0x00U, 0x00U, 0x00U, 0x00U};
    case '(': return {0x02U, 0x04U, 0x08U, 0x08U, 0x08U, 0x04U, 0x02U};
    case ')': return {0x08U, 0x04U, 0x02U, 0x02U, 0x02U, 0x04U, 0x08U};
    case '-': return {0x00U, 0x00U, 0x00U, 0x1FU, 0x00U, 0x00U, 0x00U};
    case '/': return {0x01U, 0x01U, 0x02U, 0x04U, 0x08U, 0x10U, 0x10U};
    case '!': return {0x04U, 0x04U, 0x04U, 0x04U, 0x04U, 0x00U, 0x04U};
    case '?': return {0x0EU, 0x11U, 0x01U, 0x02U, 0x04U, 0x00U, 0x04U};
    default: return {};
    }
}
...
DebugOverlayBitmap rasterizeTextRows(
    const std::vector<std::string>& rows,
    std::uint32_t scale,
    std::array<std::uint8_t, 3U> background,
    std::array<std::uint8_t, 3U> foreground) {
    return rasterizeRows(rows, scale, background, foreground);
}
