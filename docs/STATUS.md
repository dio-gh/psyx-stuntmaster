...
| Input | Working; revised mouse orientation awaiting live retest | Remappable SDL keyboard/gamepad input reaches the retail PAD buffer. Semantic gameplay mouse buttons follow the live retail controller layout and have passed initial menu-isolation validation. Two-axis mouse direction now uses the retail camera-relative stick-angle convention for facing; camera-relative strafe and character-relative movement modes retain exact gameplay/focus capture gating. The host reports port one as a DualShock so retail vibration gates open, and the game's motor values drive `SDL_GameControllerRumble` on the first attached gamepad. |
...
| Gameplay mouse mode | F10 | Cycles camera-relative movement with authored strafe animations, character-relative ordinary movement, and stock/off. Left Punch and right Kick are defaults; all eight primary actions are directly bindable. Both facing modes disable strafe auto-targeting. Automated guest-execution and transition tests pass; live feel/campaign validation is pending. |
