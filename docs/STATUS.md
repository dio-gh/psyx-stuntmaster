...
| Input | Working; mouse live validation pending | Remappable SDL keyboard/gamepad input reaches the retail PAD buffer. Semantic gameplay mouse buttons follow the live retail controller layout; mouse yaw supports camera-relative strafe and character-relative movement modes, with exact gameplay/focus capture gating. The host reports port one as a DualShock so retail vibration gates open, and the game's motor values drive `SDL_GameControllerRumble` on the first attached gamepad. |
