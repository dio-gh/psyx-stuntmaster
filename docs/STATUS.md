...
| Gameplay mouse mode | F10 | Toggles a dual-heading camera-relative mode and stock/off. Full-speed Run and Move remain retail while directional animations follow mouse-facing. A bounded velocity/acceleration controller stabilizes turning, including Punch/Kick aim during retail's opening targeting frames. Left Punch and right Kick are defaults; all eight primary actions remain semantically bindable. Thirteen ownership-aware guest seams and their transitions are emulator-tested; live campaign validation is pending. |
