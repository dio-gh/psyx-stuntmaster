...
| Input | Working; contextual mouse movement awaiting live retest | Remappable SDL keyboard/gamepad input reaches the retail PAD buffer. Semantic mouse buttons and corrected two-axis facing have passed live validation. Camera-relative Strafe substitution is limited to ordinary locomotion so launcher bounce, fall, push, and ladder handlers retain retail Move; this correction awaits live retest. Gameplay/focus capture remains exact. The host reports port one as a DualShock so retail vibration gates open, and the game's motor values drive `SDL_GameControllerRumble` on the first attached gamepad. |
