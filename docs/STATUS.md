...
| Input | Working; dual-heading mouse control awaits campaign validation | Remappable SDL keyboard/gamepad input reaches the retail PAD buffer. Semantic mouse actions preserve the live retail layout and contextual combos. Camera-relative travel remains full-speed Run while a scoped Stand/Run lease gives official body heading to a bounded mouse-turn controller; six ownership seams preserve launcher, fall, push, traversal, combat, and explicit Strafe contexts. Split-heading transitions produce on-screen and rate-limited log diagnostics. Gameplay/focus capture remains exact. The host reports port one as a DualShock so retail vibration gates open, and the game's motor values drive `SDL_GameControllerRumble` on the first attached gamepad. |
