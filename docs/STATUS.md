...
| Input | Working; dual-heading mouse control awaits campaign validation | Remappable SDL keyboard/gamepad input reaches the retail PAD buffer. Semantic mouse actions preserve the live retail layout and contextual combos. Camera-relative travel remains full-speed Run while Stand, Run, Jump, Fall, and launcher Flip lease official body heading to a bounded mouse-turn controller; twelve ownership and consumer seams preserve travel physics plus push, traversal, combat, and explicit Strafe contexts. Expected lease splits are console-only, while persistent unexpected context splits produce an on-screen diagnostic. Gameplay/focus capture remains exact. The host reports port one as a DualShock so retail vibration gates open, and the game's motor values drive `SDL_GameControllerRumble` on the first attached gamepad. |
